...
\texttt{BermudanSwaption} calibration driver, the parallel Jacobian
delivers up to \textbf{11.6$\times$} wall-clock speedup (G2,
super-linear through cache effects) and a clean \textbf{8.1$\times$}
on a HullWhite-analytic baseline that isolates \texttt{fdjac2\_parallel}
from the tree-engine's inner-pragma speedup.  Hardware counters show
IPC rising from 0.49 to 0.75 (+53\%) with cache-miss rate effectively
flat.  All parallel results --- VaR/ES vectors and calibrated model
parameters alike --- are bit-identical to the sequential baselines.
\end{abstract}
...
\midrule
HullWhite analytic  &  0.094 &  0.037 & 39.3\% \\
G2 analytic         &  0.479 &  0.316 & 65.8\% \\
HullWhite numerical & 65.761 & 32.453 & 49.3\% \\
\bottomrule
\end{tabular}
\end{table}

The Jacobian phase accounts for 39--66\% of sequential wall time
--- well within Cao et al.'s 60--80\% prediction once the per-call
cost-function cost includes the dominant tree/engine work.
...
HullWhite analytic is the \emph{clean} baseline: the Jamshidian
engine has no tree and thus no inner-pragma confound, so the
8.09$\times$ speedup is attributable purely to
\texttt{fdjac2\_parallel}.  G2 analytic at 11.61$\times$ is
super-linear because $n = 5$ Jacobian columns give both more
parallel work and better per-thread cache residency (each thread's
working set is a single-column slice of the integration grid).
HullWhite numerical gives a more modest 5.16$\times$ because its
non-Jacobian phase (baseline $f(x)$ re-evaluations between LM
steps) accounts for 50\% of sequential cost and runs serial in
\texttt{lmdif}'s main loop.
...
HullWhite analytic hits 8.12$\times$ at $T = 8$ on just $n = 2$
columns, a result that exceeds the theoretical
$\min(n, T) = 2$ ceiling.  This is possible because, in our
implementation, the $T = 1$ baseline includes not only column
evaluation but also the residual subtraction and scaling loop
inside \texttt{fdjac2}; our per-thread clones amortise these
fixed costs over cache-resident state, giving a two-way
super-linear effect on top of the expected two-way column
parallelism.  HullWhite numerical reaches 8.17$\times$ and
G2 analytic reaches 21.15$\times$ for the same reason amplified
by its $n = 5$ column count.
...
$5.2\times$ in the clean sweep of Figure~\ref{fig:e9}) is lower
than the Jacobian-phase speedup.  Third, the cache-miss \emph{rate}
...
\begin{itemize}[noitemsep]
  ...
  \item On the shipped \texttt{BermudanSwaption} calibration driver,
    the parallel Jacobian delivers \textbf{8.09$\times$} wall speedup
    on the clean HW-analytic baseline (no tree confound) and up to
    \textbf{11.6$\times$} on G2 at $T = 8$.
